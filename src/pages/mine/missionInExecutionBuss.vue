...
<div id="missionInExecutionBuss">
    <div class="main" v-if="mainShow">
        ...
        <div class="statusBox flexSpace bgW">
            ...
            <div class="right">
                <button class="cancelExecute"  v-if="!isCancel&&showData.taskType!=3&&showData.status!=4&&showData.priceType!=2"  @click="cancelExecute">取消执行</button>
            </div>
        </div>
        <!-- 每个部分 -->
        <!-- 是否有取消执行的信息 -->
        ...
        <div class="eachPart bgW">
            ...
            <div class="content">
                <div class="hang flexSpace">
                    <p class="left"><span class="title">总费用：</span><span class="mainText">{{stageDetail.price?stageDetail.price:''}}</span></p>
        ...
        <div class="btnBox bgW">
            <button  class="long_btn" @click="toEvaluationAuthor">评价</button>
            <!-- <button class="long_btn disable"  v-if="isCancel">已结束</button> -->
        </div>
    </div>
    <!-- ==============查看接收者================ -->
...
export default {
    ...
    methods: {
        ...
        getMissiondetail(id) {
            ...
            that.$http('get', that.$store.state.baseUrl + 'api/Order/' + id + "?uid=" + this.$store.state.uid).then(function (res) {
                if (res.data.code == '00') {
                    ...
                    //   console.log(that.missionCancelMsg)
                    that.$http('get', that.$store.state.baseUrl + 'api/Order/CancelTask', {
                        taskId: that.missionId
                    }).then(function (res) {
                        if (res.data.code == '00') {
                            if (res.data.data) {
                                that.missionCancelMsg = res.data.data;
                                that.receviceUid = res.data.data.uid;
                                console.log(that.receviceUid + "---" + that.$store.state.uid)
                    ...
                    console.log('ooo')
                    console.log(that.showData.authorId)
        ...
        agreeCancel(num) {
            ...
            var url = that.$store.state.baseUrl + 'api/Order/Cancel/' + that.missionCancelMsg.id;
            // console.log(url)
            ...
            that.$http('get', that.$store.state.baseUrl + 'api/Order/CancelTask', {
                ...
                if (res.data.code == '00') {
                    if (res.data.data) {
                        that.missionCancelMsg = res.data.data;
                        that.receviceUid = res.data.data.uid;
                        that.receviceUid = that.$store.state.uid == res.data.data.uid
                        var time = res.data.data.applyTime;
                        var time3;
                        time3 = new Date(time.replace(/-/g, "/")).getTime() + 1000 * 60 * 60 * 24 * 3;
                        time3 = that.format(time3)
